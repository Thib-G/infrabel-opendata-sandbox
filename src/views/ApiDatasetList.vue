  ...
  <div>
    ...
    <div v-if="datasets">
      ...
      <p>
        <a
          href
          v-if="datasets.parameters.start > 0"
          @click.prevent="start -= 10"
          @keydown.prevent="start -= 10"
        >prev</a>&nbsp;
        <a
          href
          v-if="datasets.parameters.start + datasets.datasets.length
            < datasets.nhits"
          @click.prevent="start += 10"
          @keydown.prevent="start += 10"
        >next</a>
      ...
      <table>
        ...
        <tbody>
          <tr
            v-for="d in datasets.datasets"
            :key="d.datasetid"
          >
            <td class="right">
              <a
                :href="`https://opendata.infrabel.be/explore/dataset/${d.datasetid}`"
                target="_blank"
              >{{ d.datasetid }}</a>
            </td>
            <td class="left">
